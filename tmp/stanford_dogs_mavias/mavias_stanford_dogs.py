...
class MAVIASTrainer(BaseTrainer):
    ...
    def _train_iter(self, batch):
        ...
        #     batch_groups = []
        #     for i in range(batch_dict["predictions"].shape[0]):
        #         class_name = [
        #             name
        #             for name, idx in self.class_name_to_idx.items()
        #             if idx == targets[i].item()
        #         ][0]
        #         sample_tags = tags[i]

        #         # Check if sample has any of the saved tags
        #         has_saved = has_saved_tag(
        #             sample_tags, class_name, self.tags_above_threshold
        #         )
        #         print(sample_tags, has_saved)
        #         batch_groups.append(int(has_saved))
        #     batch_dict["groups"] = torch.tensor(batch_groups, dtype=torch.long)

        #     print(batch_dict["groups"])
    ...
    def _method_specific_setups(self):
        ...
        if (
            self.cfg.DATASET.TYPE == "cifar10"
            or self.cfg.DATASET.TYPE == "stanford_dogs"
        ):
        ...
        self.class_name_to_idx = {
            class_name: idx
            for idx, class_name in enumerate(self.dataloaders["train"].dataset.classes)
        }
        # Load saved tags (either CSV or JSON file)
        with open("tags_above_threshold.json", "r") as json_file:
            self.tags_above_threshold = json.load(json_file)
