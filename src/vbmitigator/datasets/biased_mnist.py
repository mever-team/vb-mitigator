...
from vbmitigator.datasets.utils import (
    TwoCropTransform,
...
class BiasedMNIST(MNIST):
    ...
    def __init__(
        ...
        if self.load_bias_feature:
            if train_corr:
                ...
                log_msg(f"load bias feature: {bias_feature_dir}", "INFO")
        ...
        if save_path.is_dir():
            ...
            log_msg(f"use existing color_mnist from {save_path}", "INFO")
            self.data = pickle.load(open(save_path / "data_s.pkl", "rb"))
            self.targets = pickle.load(open(save_path / "targets_s.pkl", "rb"))
            self.biased_targets = pickle.load(
                open(save_path / "biased_targets_s.pkl", "rb")
            )
        else:
            self.random = True
            ...
            log_msg(f"save color_mnist to {save_path}", "INFO")

            save_path.mkdir(parents=True, exist_ok=True)
            pickle.dump(self.data, open(save_path / "data_s.pkl", "wb"))
            pickle.dump(self.targets, open(save_path / "targets_s.pkl", "wb"))
            pickle.dump(
                self.biased_targets, open(save_path / "biased_targets_s.pkl", "wb")
            )
    ...
    def __getitem__(self, index):
        ...
        return {"inputs": img, "targets": target, "background": bias, "index": index}
...
@register_dataset("biased_mnist")
def build_biased_mnist(cfg):
