...
from vbmitigator.core.utils import load_checkpoint, log_msg, save_checkpoint
...
class DebiANTrainer(BaseTrainer):
    ...
    def _setup_dataset(self):
        ...
        # DebiAN alternates two independently-shuffled streams over the *same*
        # training set. Build the second loader from the already-constructed
        # train dataset instead of rebuilding the entire dataset (which the
        # original code did via a second get_dataset() call — double the file
        # I/O, transforms setup and memory).
