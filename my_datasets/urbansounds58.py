...
def compute_or_load_embeddings_audio(
    ...
    emb_path = os.path.join(cache_dir, f"{split_name}_audio_embeddings_classes_58.npy")
    file_paths_cache_path = os.path.join(
        cache_dir, f"{split_name}_audio_file_paths_classes_58.txt"
    )
...
def get_urbansounds_dataloaders(
    ...
    df_filtered = df_filtered[df_filtered["classID"].isin([4, 8])].copy()
    print(
        f"\nFiltered DataFrame size (only classes 5 and 8, and audio exists): {len(df_filtered)} rows"
    )

    # Target: 'classID' (remapped to 0 or 1 for binary classification)
    # Class 5 -> 0, Class 8 -> 1
    df_filtered["bias"] = df_filtered["classID"].map({4: 0, 8: 1}).astype(int)

    # Bias: 'salience' (1 or 2)
    df_filtered["target"] = df_filtered["salience"].map({1: 0, 2: 1}).astype(float)
    ...
    # Test set: fold 10
    test_df = df_filtered[df_filtered["fold"] >= 8].copy()
    # Training set: rest of the folds (1-9)
    train_df = df_filtered[df_filtered["fold"] < 8].copy()
    ...
    train_df_cache_path = os.path.join(
        root, f"train_df_folds_1_9_seed_{random_seed}_classes_58.pkl"
    )
    test_df_cache_path = os.path.join(
        root, f"test_df_fold_10_seed_{random_seed}_classes_58.pkl"
    )
...
if __name__ == "__main__":
    ...
    train_loader, test_loader, _, train_dataset, test_dataset, _ = (
    ...
    print("\n--- Total Samples per Subgroup (Target & Bias) ---")
    # For training dataset
    train_targets = train_dataset.targets.numpy()
    train_biases = train_dataset.biases.numpy()
    train_df_subgroups = pd.DataFrame({"target": train_targets, "bias": train_biases})
    print("\n--- Training Set Subgroup Counts ---")
    print(train_df_subgroups.groupby(["target", "bias"]).size().unstack(fill_value=0))

    # For testing dataset
    test_targets = test_dataset.targets.numpy()
    test_biases = test_dataset.biases.numpy()
    test_df_subgroups = pd.DataFrame({"target": test_targets, "bias": test_biases})
    print("\n--- Test Set Subgroup Counts ---")
    print(test_df_subgroups.groupby(["target", "bias"]).size().unstack(fill_value=0))
